...
const SuperAdmin = () => {
  ...
  const displayStats = [
    { label: "Total Companies", value: stats?.totalCompanies ?? "0", icon: <Building size={20} /> },
    { label: "Total Users", value: stats?.totalUsers ?? "0", icon: <Users size={20} /> },
    { label: "Total Staff", value: stats?.totalStaff ?? "0", icon: <UserCheck size={20} /> },
    { label: "Company Admins", value: stats?.totalCompanyAdmins ?? "0", icon: <Briefcase size={20} /> },
    { label: "Super Admins", value: stats?.totalSuperAdmins ?? "0", icon: <Crown size={20} /> },
    { label: "Active Subs", value: stats?.activeSubscriptions ?? "0", icon: <CreditCard size={20} /> },
    { label: "Expired Plans", value: stats?.expiredPlans ?? "0", icon: <AlertCircle size={20} /> },
    { label: "Tests Used", value: `${stats?.totalTestsUsed ?? 0} / ${stats?.totalMaxTestsAllowed ?? 0}`, icon: <Zap size={20} /> },
    { label: "Active Tests Today", value: stats?.activeTestsToday ?? "0", icon: <Activity size={20} /> },
  ];
