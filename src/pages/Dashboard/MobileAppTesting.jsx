...
export default function MobileTestingDashboard() {
  ...
  const creditsBalance = useSelector(
    (state) => state.profile.user?.creditAccount?.balance ?? null,
  );
          ...
          switch (event) {
            ...
            case "PHASE_STARTED": {
              ...
              const pName = payload.phase
                ? payload.phase.charAt(0).toUpperCase() +
                  payload.phase.slice(1).toLowerCase()
                : payload.phase;
              setPhase(pName);
            ...
            case "NEW_SCREEN_DISCOVERED":
              ...
              addLog(
                `📱 Screen: ${payload.screenTitle} (${payload.elementCount} elements)`,
              );
              break;
  ...
  const startSession = async (e) => {
    ...
    try {
      ...
      if (!uploadJson.success)
        throw new Error(uploadJson.error || "APK upload failed");
  ...
  const progressWidth =
    phase === "done" || phase === "Testing"
      ...
      : phase === "Exploration"
        ? "calc(50% - 2rem)"
        : phase === "Validation" || phase === "starting"
          ? "0"
          : "0";

  const progressColor =
    phase === "done" ? "#10b981" : phase === "failed" ? "#f43f5e" : "#f97316";
            ...
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">
                  This will terminate the active test session immediately.
                </p>
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto space-y-6 pb-12 pt-4 px-4">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-1.5 text-sm">
                Upload your APK and launch the autonomous AI testing agent.
              </p>
            </div>

            <div className="flex flex-col items-start md:items-end gap-2 shrink-0">
              <div
                className={`flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border ${statusConfig.pill}`}
              >
              ...
              {sessionId && (
                <div
                  className={`flex items-center gap-1.5 text-xs font-semibold ${wsConnected ? "text-emerald-600" : "text-amber-500"}`}
                >
          ...
          <form onSubmit={startSession} className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div
                onDragOver={(e) => {
                  e.preventDefault();
                  if (!isSessionActive) setIsDragOver(true);
                }}
                ...
                onClick={() =>
                  !isSessionActive && fileInputRef.current?.click()
                }
              ...
              >
                ...
                {apkFile ? (
                  ...
                  <>
                    <div
                      className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 transition-colors ${isDragOver ? "bg-orange-200" : "bg-orange-50"}`}
                    >
                      <Upload
                        size={20}
                        className={
                          isDragOver ? "text-orange-600" : "text-orange-400"
                        }
                      />
                    </div>
                    <div>
                      <div className="text-sm font-bold text-slate-700">
                        {isDragOver
                          ? "Drop your APK here"
                          : "Drag & drop APK or click to browse"}
                      </div>
                      <div className="text-xs text-slate-400 mt-0.5">
                        Supports .apk files only
                      </div>
                    </div>
                  </>
                )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
          <div className="md:col-span-5 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-5 text-center">
              Pipeline Status
            </h2>
            <div className="flex justify-between items-start px-4 relative max-w-[260px] mx-auto w-full">
              ...
              {PHASE_ORDER.map((step) => {
                ...
                return (
                  <div
                    key={step}
                    className="flex flex-col items-center gap-2 relative z-10 bg-white px-2"
                  >
                    <div
                      className={`w-[15px] h-[15px] rounded-full border-2 border-white shadow transition-all duration-300 ${phaseDotClass[status]}`}
                    />
                    <span
                      className={`text-[9px] font-bold uppercase tracking-wider transition-colors ${phaseTextClass[status]}`}
                    >
                      {step}
                    </span>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 h-[600px]">
          ...
          >
            ...
            <div className="absolute top-0 inset-x-0 flex items-center justify-between px-4 py-2.5 bg-slate-900/80 backdrop-blur-sm border-b border-slate-700/40 z-20">
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${wsConnected && isSessionActive ? "bg-emerald-400 animate-pulse" : "bg-slate-600"}`}
                />
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl border border-slate-200 shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 relative min-h-0">
              {/* LOGS tab */}
              <div
                className={`absolute inset-0 bg-[#0f172a] overflow-y-auto hide-scroll p-5 font-mono text-xs flex flex-col ${activeTab === "logs" ? "" : "hidden"}`}
              >
                {logs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center flex-1 text-slate-600 gap-3">
                    ...
                    <p className="italic text-sm">
                      System ready — awaiting session start…
                    </p>
                  </div>
                ) : (
                  <div className="space-y-1 pb-3">
                    {logs.map((log, i) => (
                      ...
                      >
                        <span className="text-slate-600 shrink-0 tabular-nums text-[10px] pt-px">
                          {log.ts}
                        </span>
              ...
              <div
                className={`absolute inset-0 bg-slate-50 overflow-y-auto hide-scroll flex flex-col ${activeTab === "results" ? "" : "hidden"}`}
              >
                  ...
                  <div className="p-5 flex flex-col items-center justify-center flex-1 gap-5">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        All 3 phases completed — your Excel report is available
                      </p>
                    </div>
                    <div className="grid grid-cols-3 gap-3 w-full max-w-xs">
                      ...
                      <div className="bg-emerald-50 rounded-2xl p-3 text-center border border-emerald-100 shadow-sm">
                        ...
                        <div className="text-[9px] font-bold text-emerald-500 uppercase mt-0.5">
                          Passed
                        </div>
                      </div>
                      <div className="bg-rose-50 rounded-2xl p-3 text-center border border-rose-100 shadow-sm">
                        ...
                        <div className="text-[9px] font-bold text-rose-400 uppercase mt-0.5">
                          Failed
                        </div>
                      </div>
                    </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                            ...
                            <span
                              className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${ass.passed ? "text-emerald-700 bg-emerald-50" : "text-rose-600 bg-rose-50"}`}
                            >
                  ...
                  phase !== "done" && (
                    <div className="flex flex-col items-center justify-center flex-1 text-slate-400 gap-3 p-5">
                      ...
                      <p className="text-xs text-slate-300 text-center">
                        Test assertions appear here as the agent runs
                      </p>
                    </div>
                  )
                )}
