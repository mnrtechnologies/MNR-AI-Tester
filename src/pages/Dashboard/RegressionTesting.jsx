...
const MLPipelineTab = ({
  ...
  <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 pt-4 flex justify-center">
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 flex flex-col justify-center items-center text-center max-w-xl w-full">
      ...
      {/* Conditionally Render the Backend Output Only If Available */}
      {pipelineResult && (
        <div className="mt-8 w-full animate-in fade-in border-t border-slate-100 pt-6 text-left">
          <div className="flex flex-col gap-3">
            <p className="text-sm text-slate-600">
              ...
              <span className="text-emerald-600 font-medium capitalize">
                {pipelineResult.status?.replace("_", " ")}
              </span>
            </p>
            <p className="text-sm text-slate-600">
              <span className="font-bold text-slate-800 uppercase tracking-wider text-xs">
              ...
              <span className="text-orange-600 font-black text-lg">
                {pipelineResult.raw_metrics_available}
              </span>
            </p>
            <div className="mt-2 bg-slate-50 p-4 rounded-xl border border-slate-200 text-sm text-slate-600 font-medium">
...
const AIChatTab = ({
  ...
  <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col h-[calc(100vh-160px)] sm:h-[calc(100vh-200px)] animate-in fade-in slide-in-from-bottom-2 pt-4">
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-slate-50/30">
      ...
      {chatMessages.map((msg, i) => (
        <div
          key={i}
          className={`flex flex-col max-w-[90%] sm:max-w-[80%] ${msg.role === "user" ? "ml-auto" : "mr-auto"}`}
        >
          <div
    ...
    <form
      ...
      className="p-3 sm:p-4 bg-white border-t border-slate-100 flex gap-2 sm:gap-3"
    >
      <input
        type="text"
        value={chatInput}
...
const PerformanceDashboard = () => {
  ...
  // Chat States - Starts strictly empty without dummy prompts
  ...
  const handleRunPipeline = async () => {
    ...
    setPipelineResult(null); // Clear previous runs
    ...
    try {
      ...
      addToast(data.message || "Pipeline started", "success");
    } catch (e) {
      addToast("Error: " + e.message, "error");
    } finally {
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          text: data.answer,
          meta: `${data.insights_used} insights referenced`,
        },
      ]);
    } catch (e) {
      setChatMessages((prev) => [
