...
import { Lock, AlertCircle, Mail, AlertTriangle } from "lucide-react";
import { Link } from "react-router-dom";

/**
 * Gates a testing surface on the company's credit balance.
 *
 * Three states, deliberately not treated the same:
...
const SubscriptionGuard = ({ children, featureName = "this feature", requiredCredits = 1 }) => {
  ...
  const isExpired = !account || account.isActive === false;
  ...
  const isOutOfCredits =
    !isExpired && !unlimited && !account?.overageEnabled && balance < requiredCredits;

  const threshold = account?.lowCreditThreshold ?? 0;
  const isLow = !isExpired && !isOutOfCredits && !unlimited && threshold > 0 && balance <= threshold;
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center p-4 z-10">
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 max-w-md w-full flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">
            {isExpired ? "Subscription Expired" : "Out of Credits"}
          </h2>
